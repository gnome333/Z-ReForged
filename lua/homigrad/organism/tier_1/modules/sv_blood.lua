...
module[2] = function(owner, org, mulTime)
	...
	if #org.wounds > 0 then
		...
		for i, wound in pairs(org.wounds) do
			...
			local bleed = rand1 * wound[1] * mulTime * math.max(org.pulse, 20) / 70 * 2.0 * (1 - math.min(6, 0.5)) * org.bleedingmul * 0.02
			local coagulate = 2 * mulTime * rand2 * (0.1 + 1) * 0.04-- / #org.wounds
